...
| `radio-frame.test.mjs` | 11 `node:test` cases: round-trips, 32/33-byte boundary, every rejection rule, dedup, sequence wrap, limiter. |
...
| `index.html` | Browser harness: mood row, optional note with byte budget, Send (Preparing → Playing → Broadcast played), Preview, Software loopback, Listen (mic only on click), Stop / Esc teardown, last-10 inbox tagged unverified. |
...
- Alias is 4 random bytes in `localStorage`, with a visible reset. The message id wraps at 65535 and the alias rotates.
