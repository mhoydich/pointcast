...
export const GET: APIRoute = async () => {
  ...
  const manifest = {
    ...
    roadmap: {
      ...
      'v0.39': 'TV mode · Phase 2 (personal channel). New /sparrow/tv/saved — the personal-list variant of /sparrow/tv. Same ten-foot chrome (slide rotation, channel-tinted wash, surface-detect, here-strip, vmin-scaled type), but the slide pool is hydrated from the local sparrow:saved reading list (newest-first). Privacy first: the personal list is private-default-blocked, so a TV in a shared space (kitchen, hotel room, friend\'s place) doesn\'t paint a stranger\'s reading list onto the wall. Three states: (1) gate — until sparrow:tv-saved-ok="1" the surface paints a calm "this screen is private" card with the opt-in instruction (or open the URL with ?saved-ok=1); (2) empty — gate open but sparrow:saved is empty, paints a CTA pointing at /sparrow + the S keyboard shortcut; (3) personal — gate open + saved IDs available, paints the slides with a "★ saved by you" attribution chip. Saved strip (.sptv-saved-strip) replaces the rosette: total saved count + per-channel mini-bar showing distribution across the nine channels. ?saved-ok=1 query writes the flag and strips the param; storage events trigger a re-route so toggling the gate from another tab updates the wall TV in real time. SW_VERSION → sparrow-v0.39.0; /sparrow/tv/saved added to SHELL_URLS for offline-first availability (gate is enforced at runtime, not in the cache). (current)',
