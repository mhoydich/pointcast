  ...
  const manifest = {
    ...
    nostr: {
      ...
      federated_lists: {
        ...
        digest_sidecar: {
          ...
          cron_worker: {
            since: 'v0.33',
            status: 'v0.36 · full signals aggregation live in email body (top co-saved + channel distribution) + one-shot block-lookup prefetch from /blocks.json per cron tick',
            location: 'workers/sparrow-digest/ (wrangler.toml + src/{index,signing,send,nostr,aggregate}.ts + README.md)',
            ...
            aggregation: 'workers/sparrow-digest/src/aggregate.ts — pure-function port of /sparrow/signals Panel 1 + Panel 3. aggregate(newestPerAuthor, lookup) → { activeFriends, totalSaves, topCoSaved[≥2], channelDist }. First-picker wins via earliest created_at among savers. Rendered as HTML table + plain-text mirror.',
            block_lookup: 'Pre-fetch /blocks.json ONCE per cron tick (with cf.cacheTtl:300) and pass {[id]: {title, channel, channel_name}} into every subscriber\'s renderDigestEmail. Beats 300+ /b/<id>.json requests per subscriber. Returns {} on failure — aggregation degrades to "unknown block" rows, email still sends.',
            email_body: 'Subscribers with npub + relays AND ≥2 active friends: top co-saved table (count badge · № id · title · channel chip) + channel distribution bars (progress + percent). Subscribers with <2 active friends still get the v0.35 one-line summary. Subscribers without npub still get the v0.30 "open signals" prompt.',
            dry_run: 'GET /dry-run on the worker returns {total, due, now, worker_version:"v0.36"} without sending',
            not_yet: 'dead-letter bucket for repeated retriable failures · /ops/dead-letter endpoint for human review · native Sparrow.app NIP-01 kind-0 profile lookup',
          },
        ...
        // Decoupled from friends.astro so curation is a data edit, not
        // a code deploy. friends.astro fetches this on boot and falls
        // back to a tiny internal seed on failure.
        federation_json: {
          since: 'v0.32',
    ...
    reader_state_mirror: {
      ...
      payload_shape: {
        ...
        friends:   { value: 'Array<{ pubkey: hex-64, alias?: string (≤40), muted?: bool }>', updated_at: 'ISO 8601', since: 'v0.32' },
        profiles:  { value: '{ [hex: string]: { name?, display_name?, picture?, nip05?, nip05_verified?, nip05_verified_at?, fetched_at } }', updated_at: 'ISO 8601', since: 'v0.32' },
      },
      ...
      web_state_keys: [
        ...
        'sparrow:friends', 'sparrow:friends:updated_at',
        'sparrow:profiles', 'sparrow:profiles:updated_at',
      ],
    ...
    roadmap: {
      ...
      'v0.35': 'Cron worker gains a Nostr client + retry-with-jitter + live friends-saved summary. workers/sparrow-digest/src/nostr.ts ports NostrRelayClient.swift to TypeScript (Web WebSocket + JSON frames, no deps). collectFromRelay + collectAcrossRelays + newestPerAuthorByDTag helpers · 6s timeout per relay · 500-event safety cap. workers/sparrow-digest/src/send.ts extracts MailChannels transport with retry (3 attempts max, 800ms/1600ms/3200ms exponential-jitter backoff), 5xx/429/408 retriable, 401/403/422 fail-fast without touching last_sent_at so ops sees them + next cron retries. renderDigestEmail now fetches each subscriber\'s kind-3 contact list + their friends\' kind-30078 public saved events, surfacing "N of M followed signers published public saved lists · K total saved-block references" in the email body (text + HTML). Subscribers without npub get the existing short prompt. Full signals aggregation (co-saves + channel distribution) queued for v0.36.',
      'v0.36': 'Full signals aggregation in the digest. workers/sparrow-digest/src/aggregate.ts — pure-function port of /sparrow/signals Panels 1 + 3: top co-saved (count ≥ 2, desc, with first-picker attribution) + channel distribution (top 5 channels with share bars). Email body now carries an HTML table of co-saved blocks (×N badge · № id · title · channel chip) and a progress-bar channel breakdown (plus plain-text mirrors for MailChannels). Block-lookup prefetch: /blocks.json fetched ONCE per cron tick with cf.cacheTtl:300 and passed to every subscriber\'s renderDigestEmail; beats 300+ /b/<id>.json requests per subscriber. Aggregation shows when ≥2 active friends exist; fewer than that still get the v0.35 summary line; no-npub subscribers still get the v0.30 prompt. (current)',
