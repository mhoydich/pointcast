 ...
 async function start(){
  ...
  call.actions=createVoiceActions({
   ...
   onResult:(action,result)=>{
    ...
    if("imageUrl" in result){setImages(old=>old.some(image=>image.url===result.imageUrl)?old:[{url:result.imageUrl,prompt:action.prompt},...old].slice(0,2));setImageState("Made from your spoken request · OpenAI");}
