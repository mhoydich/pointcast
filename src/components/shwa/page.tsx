...
type Call = {peer:RTCPeerConnection;channel:RTCDataChannel;mic?:MediaStream;control?:Control;audio:HTMLAudioElement;context:AudioContext;timer?:ReturnType<typeof setInterval>;timeout?:ReturnType<typeof setTimeout>;frame?:number;input?:AnalyserNode;output?:AnalyserNode;usage:number;costBase:number;closed:boolean;cancelled:boolean};
export default function Home(){
 const [phase,setPhase]=useState<Phase>("idle"),[line,setLine]=useState<Line|null>(null),[message,setMessage]=useState("Checking the line…"),[muted,setMuted]=useState(false),[seconds,setSeconds]=useState(120),[needsPlay,setNeedsPlay]=useState(false);
 const [captions,setCaptions]=useState<Caption[]>([]),[notes,setNotes]=useState<Notes|null>(null),[notesState,setNotesState]=useState("Waiting for a conversation"),[costs,setCosts]=useState({voice:0,notes:0,images:0,research:0}),[imagePrompt,setImagePrompt]=useState(""),[images,setImages]=useState<{url:string;prompt:string}[]>([]),[imageState,setImageState]=useState(""),[imageBusy,setImageBusy]=useState(false),[hasControl,setHasControl]=useState(false),[signals,setSignals]=useState({input:0,output:0,inputPath:"0,36 320,36",outputPath:"0,36 320,36"});
 const [canvas,setCanvas]=useState<CanvasItem[]>([]),[canvasEnabled,setCanvasEnabled]=useState(true),[contextState,setContextState]=useState("Shwa will receive station context when the call connects.");
 const canvasRef=useRef<CanvasItem[]>([]),canvasEnabledRef=useRef(true),activityRef=useRef<Record<string,string>>({station:"Shwa room inside PointCast. You are Shwa. The scenery is a floating green island, peach sunset clouds and a purple portal. A small call dock stays at the bottom; images are top right on desktop, and Canvas/Images/Music tabs open sheets on phones. No outside action is authorized by canvas choices."}),contextSent=useRef<Record<string,string>>({}),contextPending=useRef(false),imagePending=useRef(false);
 ...
 const cleanup=useCallback((call:Call)=>{
  ...
  call.mic?.getTracks().forEach(t=>t.stop());call.audio.pause();call.audio.srcObject=null;
  ...
  orb.current?.style.setProperty("--level","0");setMuted(false);setNeedsPlay(false);setSignals({input:0,output:0,inputPath:"0,36 320,36",outputPath:"0,36 320,36"});
 },[]);
 ...
 useEffect(()=>{
  ...
  const leave=()=>{const call=active.current;if(call){call.cancelled=true;if(call.channel.readyState==="open")call.channel.send(JSON.stringify({type:"session.close"}));closeRemote(call.control,true);cleanup(call);}studioControl.current=undefined;setHasControl(false);phaseRef.current="ended";setPhase("ended");};
 ...
 useEffect(()=>{
  const timer=setInterval(()=>{
   const control=studioControl.current,spoken=transcriptText(captionsRef.current),text=spoken+"\nSTATION CONTEXT: "+Object.entries(activityRef.current).filter(([key])=>key!=="station").map(([key,value])=>key+": "+value).join("\n").slice(-2500)+"\nALREADY ON CANVAS: "+canvasRef.current.map(c=>c.title).join(" | ").slice(-1000)+(!canvasEnabledRef.current?"\nCanvas paused: return card null.":"");
 ...
 async function generateImage(requestedPrompt?:string,cardId?:string){
  const control=studioControl.current,prompt=(requestedPrompt??imagePrompt).trim();if(!control||!prompt||imagePending.current)return;
  ...
  catch(error){if(studioControl.current===control)setImageState(error instanceof Error?error.message:"Image generation is unavailable.");if(cardId)throw error;}finally{imagePending.current=false;setImageBusy(false);}
 }
 ...
 async function start(){
  ...
  studioControl.current=undefined;setHasControl(false);setCanvas([]);canvasRef.current=[];contextSent.current={};for(const key of ["canvas","choice","research","image"])delete activityRef.current[key];setContextState("Waiting to share station context with Shwa.");captionsRef.current=[];setCaptions([]);setNotes(null);setNotesState("Waiting for a conversation");notesDigest.current="";notesStopped.current=false;notesFinal.current=false;eventsSeen.current.clear();setImages([]);setImageState("");setImagePrompt("");
  setPhase("connecting");setMessage("Allow your microphone to say hello.");setSeconds(120);setMuted(false);
  ...
  try{
   ...
   audio.autoplay=true;void context.resume().catch(()=>{});
   peer.addEventListener("track",event=>{
    if(call.closed||active.current!==call){event.track.stop();return;}
    const stream=new MediaStream([event.track]);audio.srcObject=stream;void audio.play().catch(()=>setNeedsPlay(true));
   ...
   channel.addEventListener("message",({data})=>{
    ...
    if(event.type==="session.started"&&!call.cancelled){setPhase("connected");clearTimeout(call.timeout);setMessage("You’re connected. Think out loud.");}
   ...
   const response=await fetch(GATEWAY+"/session",{method:"POST",credentials:"omit",headers:{"Content-Type":"application/json"},body:JSON.stringify({sdp:peer.localDescription?.sdp}),signal:AbortSignal.timeout(35000)});
 ...
 return <StationPanels canvas={canvas} canvasEnabled={canvasEnabled} setCanvasEnabled={setCanvasEnabled} contextState={contextState} onActivity={activity} onAnswer={answerCard} onCanvasImage={card=>generateImage(card.prompt,card.id)} onResearch={researchCard} phase={phase} line={line} message={message} seconds={seconds} muted={muted} signals={signals} captions={captions} notes={notes} notesState={notesState} costs={costs} prompt={imagePrompt} setPrompt={setImagePrompt} images={images} imageState={imageState} imageBusy={imageBusy} canGenerate={hasControl} generate={()=>void generateImage()}>
  ...
  <div className="call-actions">
   {connected?<><button className={"mute-button"+(muted?" muted":"")} onClick={toggleMute} aria-pressed={muted} aria-label={muted?"Turn microphone on":"Mute microphone"}>{muted?<MicOff size={21}/>:<Mic size={21}/>}</button><button className="call-button end-button" onClick={end}><PhoneOff size={20}/>End call</button></>:busy?<button className="call-button secondary-button" onClick={end} disabled={phase==="ending"}><PhoneOff size={20}/>{phase==="ending"?"Ending…":"Cancel"}</button>:<button className="call-button" disabled={!line?.available||imageBusy} onClick={()=>void start()}><Phone size={21}/>{phase==="ended"?"Call again":"Start a call"}</button>}
   {!busy&&!connected&&!line?.available&&<button className="refresh-button" onClick={()=>void refresh()}>Check the line</button>}
  </div>
  {needsPlay&&<button className="sound-button" onClick={()=>{void active.current?.audio.play().then(()=>setNeedsPlay(false)).catch(()=>{});}}><Volume2 size={18}/>Tap to hear Shwa</button>}
 </StationPanels>;
}
