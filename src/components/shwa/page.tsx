...
export default function Home(){
 ...
 const canvasRef=useRef<CanvasItem[]>([]),canvasEnabledRef=useRef(true),activityRef=useRef<Record<string,string>>({station:"Shwa room inside PointCast. You are Shwa. A desktop board holds conversation pieces. Studio, Grid, Reading and Radio backgrounds change the view. A small call dock stays at the bottom; images are top right with explicit Greenlight controls underneath. Radio animation shows audio activity or a labeled ambient preview; it does not broadcast to others. No outside action is authorized by canvas choices."}),contextSent=useRef<Record<string,string>>({}),contextPending=useRef(false),imagePending=useRef(false);
 ...
 async function start(){
  ...
  studioControl.current=undefined;setHasControl(false);setCanvas([]);canvasRef.current=[];contextSent.current={};for(const key of ["canvas","choice","research","image"])delete activityRef.current[key];setContextState("Waiting to share station context with Shwa.");captionsRef.current=[];setCaptions([]);setNotes(null);setNotesState("Waiting for a conversation");notesDigest.current="";notesStopped.current=false;notesFinal.current=false;eventsSeen.current.clear();setImages([]);setImageState("");
