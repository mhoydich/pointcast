 ...
 async function start(){
  ...
  call.actions=createVoiceActions({
   ...
   onResult:(action,result)=>{
    ...
    setCanvas(old=>old.map(card=>card.id===action.card.id?{...card,title:action.name==="search_web"?"Here’s what turned up":"Your image is here",workState:"complete",workMessage:"",...("research" in result?{researchResult:result.research,researchedAt:Date.now()}:{imageUrl:result.imageUrl})}:card));
    if("imageUrl" in result){setImages(old=>[{url:result.imageUrl,prompt:action.prompt},...old].slice(0,2));setImageState("Made from your spoken request · OpenAI");}
