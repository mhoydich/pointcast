...
import { daySeed, todayPT } from './daily';
...
export interface ChartOfTheDay {
  slug: string;
  title: string;
  kicker: string;
  dek: string;
...
function countByDate(blocks: BlockEntry[]): Map<string, { count: number; latestId: string | null }> {
  const out = new Map<string, { count: number; latestId: string | null }>();
...
export function getChartOfTheDay(blocks: BlockEntry[], now = new Date()): ChartOfTheDay {
  const windowDays = 14;
  ...
  const trendLabel = delta === 0
    ...
    : `${delta > 0 ? '+' : ''}${delta} versus yesterday`;
  ...
  const insight = todayPoint.value === 0
    ? 'No new block has landed yet today. The chart will lift as soon as the next receipt ships.'
    : trend === 'up'
      ? `Today is ahead of yesterday by ${delta} ${Math.abs(delta) === 1 ? 'block' : 'blocks'}.`
      : trend === 'down'
        ? `Today is quieter than yesterday by ${Math.abs(delta)} ${Math.abs(delta) === 1 ? 'block' : 'blocks'}.`
