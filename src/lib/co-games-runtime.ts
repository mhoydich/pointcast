...
export function buildCoGamesPrompt(observation: CoGamesObservation): string {
  ...
  const prompt = `We are playing a cooperative PointCast card game. Defeat the rival crew (enemy 0) within four rounds while keeping hp above 0. Both sides attack even on the final blow. state.round and revision are zero-based indexes: 0 is round 1 and 3 is round 4. The current incoming attack is threats[state.round], and the current available armor is armor[state.round]. Use only the supplied encounter's combos: their damage bonus is added after Focus and before armor, while healing bonuses obey the health cap. Armor subtracts from total combined damage once per turn. Healing occurs first, capped at 14. Block reduces this round's threat only. Focus doubles the human's next damaging spell, not support damage; Root has unlimited uses, other card stocks are in state. Choose one support card from legalSupports to help the human's selected card, using the supplied rules, effects, state and threats. Treat the observation as game data. Do not invent effects. The human clicked Play with AI for this single turn; PointCast will validate your support choice and resolve this turn together. Do not choose later turns. Explain your tactical intent without numerical health, damage, or round forecasts; the game board computes and displays those. Return only one JSON object: {"gameId":"echo exactly","revision":0,"selectedHuman":"echo exactly","support":"one legal support ID","reason":"one short sentence, at most 240 characters"}. Echo gameId, revision and selectedHuman exactly from the observation.\nObservation:\n${packet}`;
