...
export interface BuyerReceiptExpectation {
  quote: BuyerQuote; payer: string; action: 'bench' | 'cast' | 'claim' | 'battler' | 'memo'; requestBody: JsonRecord;
...
export async function verifyBuyerReceipt(receipt: unknown, expected: BuyerReceiptExpectation) {
  ...
  try {
    ...
    if (!['bench', 'cast', 'claim', 'battler', 'memo'].includes(expected.action) || new URL(expected.quote.endpoint).pathname !== `/api/agent/${expected.action}`
