import {
  BEACON_CENTER,
  RADIUS_METERS as BEACON_RADIUS_METERS,
  RADIUS_MILES as BEACON_RADIUS_MILES,
} from './neighborhoods';

export const LOCAL_AREA_RADIUS = {
  name: 'PointCast participation radius',
  label: '25-mile local layer',
  radiusMiles: BEACON_RADIUS_MILES,
  radiusMeters: BEACON_RADIUS_METERS,
  anchor: {
  ...
  policy:
    'Start with the 25-mile Beacon radius for anything that needs people to meet, trade, play, teach, or bring something physical. Keep the 100-mile Local lens for broadcast, stations, and broader Southern California context.',
} as const;
...
export const LOCAL_AREAS: LocalArea[] = [
  {
    ...
    status: 'pilot',
    description:
      'A local paddle shelf for South Bay pickleball players: register a profile, name the paddle you play, list paddles you would trade, lend, try, or donate into the library.',
    radiusFit:
      'Twenty-five miles is the right start because paddles are physical, trial windows are short, and court handoffs work best inside a Saturday-morning drive.',
  ...
  },
  {
    ...
    status: 'forming',
    description:
      'A simple page for Mike Hoydich led events: court office hours, beach walks, build nights, tasting tables, and small local sessions with a clear place to RSVP.',
    radiusFit:
      'The radius keeps the invite human: close enough for repeat attendance, wide enough to include South Bay, Westside, harbor, and downtown-edge friends.',
  ...
  },
  {
    ...
    status: 'forming',
    description:
      'A warm local league for reciprocal acts: play a match, lend a paddle, bring local honey, host a tiny lesson, publish a useful note, and keep the standings kind.',
    radiusFit:
      'A league needs repeat contact and trust. Twenty-five miles gives it a coastal circuit without becoming a spreadsheet for strangers.',
    actions: ['season ladder', 'honey table', 'service points', 'local notes'],
    palette: { ink: '#9a5f0b', wash: '#fff0bf', rule: '#e2a93e' },
  },
];

export type PaddleExchangeMode = {
  id: string;
  label: string;
  promise: string;
  signal: string;
...
export const PADDLE_EXCHANGE_MODES: PaddleExchangeMode[] = [
  {
    id: 'try',
...
export const PADDLE_PROFILE_FIELDS = [
...
export const PADDLE_LIBRARY_SLOTS = [
  {
...
export const MEETUP_SERIES: MeetupSeries[] = [
  {
...
export const UNIVERSITY_PARTICIPATION_TIERS = [
  {
    name: 'Auditor',
...
export const FIRST_TIDE_FORMAT = [
...
export const HONEY_LEAGUE_POINTS = [
...
export const HONEY_LEAGUE_SEASON = {
  name: 'Season Zero',
  length: '6 weeks',
