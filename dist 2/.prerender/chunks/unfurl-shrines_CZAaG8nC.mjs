...
const shrineSetBySlug = /* @__PURE__ */ new Map();
SHRINE_SETS.forEach((set) => {
  set.slugs.forEach((slug) => shrineSetBySlug.set(slug, set.slug));
});
const UNFURL_SHRINES = SHRINE_ITEMS.map((shrine) => ({
  ...shrine,
  miniPath: `/u/${shrine.slug}`,
  shrineSet: shrineSetBySlug.get(shrine.slug)
}));
function getShrineSet(slug) {
  const setSlug = shrineSetBySlug.get(slug);
  return SHRINE_SETS.find((set) => set.slug === setSlug);
}
function getMiniShrineDescription(shrine) {
  return `Mini shrine for ${shrine.title}: ${shrine.description}`;
}

export { SITE_URL as S, UNFURL_SHRINES as U, absoluteUrl as a, absoluteImage as b, SHRINE_SETS as c, getMiniShrineDescription as d, getShrineSet as g };
