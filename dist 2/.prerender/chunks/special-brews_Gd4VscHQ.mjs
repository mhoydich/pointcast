...
function getBrewForDate(d = /* @__PURE__ */ new Date()) {
  const y = d.getUTCFullYear();
  const start = Date.UTC(y, 0, 1);
  const dayOfYear = Math.floor((d.getTime() - start) / 864e5) + 1;
  const idx = (y * 7 + dayOfYear * 13) % BREWS.length;
  return BREWS[idx];
}
function getBrewForDayOffset(offsetDays) {
  const d = /* @__PURE__ */ new Date();
  d.setUTCDate(d.getUTCDate() + offsetDays);
  return getBrewForDate(d);
}

export { BREWS as B, getBrewForDayOffset as a, getBrewForDate as g };
