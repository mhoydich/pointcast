...
const GET = async () => {
  const body = {
    ...
    scenes: [
      ...
      { id: "tessellate", name: "TESSELLATE", description: "Hex-tile pattern that fills the viewport. Each tile picks a palette swatch slot; ~4 random tiles shift slots every 700ms so the pattern slowly drifts. Per-tile breathing alpha keeps the surface alive." }
    ],
    soundscapes: [
      ...
      { id: "markov", name: "MARKOV", description: "First-order Markov chain over a palette pentatonic with a hand-tuned transition matrix. Tunes form, evolve, and recur. 380-1100ms note intervals. Sine pad fifth bed for body." }
    ],
    palettes: PALETTES,
    clockDefault: {
    ...
    storage: {
      ...
      "pc:tide:mode": "last scene (waves|starfield|mystify|bounce|pipes|tessellate)",
      "pc:tide:soundscape": "last soundscape (drift|chimes|bubbles|granular|markov)",
    ...
    today: {
      ...
      description: "A fresh visitor with no saved preferences and no URL hash gets palette+scene from hour-of-day and soundscape from day-of-year. Shown as a TIDE OF THE DAY chip in the footer until any user interaction."
    },
    version: 4,
    versions: {
      ...
      "v4.1": "shipped 2026-04-29 — tide-of-the-day defaults (clock-derived scene, day-of-year-derived soundscape) + /tide/today.json manifest + /tide/preview gallery (40 still mini-cards covering all 8 palettes × 5 scenes).",
      "v4.2": "shipped 2026-04-29 — TESSELLATE scene (hex-tile pattern with slow slot-shift drift and per-tile breathing alpha) + MARKOV soundscape (first-order Markov chain over palette pentatonic with hand-tuned transition matrix, sine pad fifth bed)."
    },
    license: "CC0",
