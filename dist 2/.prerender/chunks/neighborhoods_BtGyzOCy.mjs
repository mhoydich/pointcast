...
const NEIGHBORHOODS_BY_DISTANCE = [...NEIGHBORHOODS].sort((a, b) => a.distance - b.distance);
const RADIUS_MILES = 25;
const RADIUS_METERS = Math.round(RADIUS_MILES * 1609.344);
const BEACON_CENTER = {
  lat: 33.9194,
  lng: -118.4088,
  placeName: "El Segundo, California, USA"
};
function countByStatus(list = NEIGHBORHOODS) {
  const c = { seed: 0, target: 0, adjacent: 0 };
  for (const n of list) c[n.status] += 1;
  return c;
}

export { BEACON_CENTER as B, NEIGHBORHOODS_BY_DISTANCE as N, RADIUS_MILES as R, RADIUS_METERS as a, countByStatus as c };
