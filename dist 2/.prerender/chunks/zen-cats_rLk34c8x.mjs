...
import { a as ZEN_CAT_WORLD_COLLECTIBLES, b as ZEN_CAT_WORLD_STYLE_PROMPT, Z as ZEN_CAT_GENESIS_COLLECTIBLES } from './zen-cat-collectibles_BLrr4dOT.mjs';

const ZEN_CATS_VERSION = "0.1.0";
const ZEN_CATS_SYMBOL = "PCCAT";
const ZEN_CATS_STORAGE_KEYS = {
...
const ZEN_CAT_CAPITAL_STORAGE_KEYS = {
...
const ZEN_CAT_REFERRAL_MILESTONES = [
  {
    count: 1,
    label: "First Signal",
...
const journeyPrompt = (title, motif, lesson, palette) => `Zen Cats Journey Print collectible, sophisticated textile and gouache art-card style, quiet luxury, engraved cat fur lines, subtle Bitcoin and Tezos geometry as abstract motifs only, no logo, no text, no financial promise. Title concept: ${title}. Motif: ${motif}. Lesson: ${lesson}. Palette: ${palette}.`;
const ZEN_CAT_JOURNEY_SERIES = [
  {
    id: "journey-001",
    number: 1,
    title: "Seed Signal",
...
function todayZenCat(now = /* @__PURE__ */ new Date()) {
  return zenCatForDate(todayPT(now));
}
function recentZenCats(count = 9, now = /* @__PURE__ */ new Date()) {
  const [year, month, day] = todayPT(now).split("-").map((part) => Number(part));
  const current = new Date(Date.UTC(year, month - 1, day));
  return Array.from({ length: count }, (_, index) => {
    const date = new Date(current.getTime() - index * 864e5);
    const iso = date.toISOString().slice(0, 10);
    return zenCatForDate(iso);
  });
}
function buildZenCatsManifest(now = /* @__PURE__ */ new Date()) {
  const today = todayZenCat(now);
  return {
    version: ZEN_CATS_VERSION,
    title: "Zen Cat Garden",
    description: "A daily PointCast collection game: care for one deterministic zen cat, collect it locally, and mint it on Tezos when the dedicated PCCAT contract is deployed.",
...
export { ZEN_CAT_JOURNEY_SERIES as Z, ZEN_CAT_REFERRAL_MILESTONES as a, buildZenCatsManifest as b, ZEN_CAT_CAPITAL_PATHWAY as c, ZEN_CAT_CAPITAL_STORAGE_KEYS as d, ZEN_CATS_STORAGE_KEYS as e, ZEN_CAT_RITUALS as f, ZEN_CATS_SYMBOL as g, recentZenCats as r, todayZenCat as t };
