...
import { b as buildMorningOceanManifest } from './morning-ocean_0Dle8vCr.mjs';
...
const GET = async () => {
  ...
  const payload = {
    ...
    contract: {
      ...
      note: "Morning Ocean is metadata-ready. Minting becomes live after a dedicated PCOCEAN FA2 contract is originated and its KT1 is pasted into contracts.json."
    },
    agentProtocol: {
      localState: "Local collection state lives under pc:morning-ocean:collection. Do not infer ownership unless the visitor supplies local or on-chain state.",
      tezos: "When contract.live is false, describe the series as Tezos-ready rather than mint-live.",
      metadata: "Token metadata is available at /api/morning-ocean-metadata/{tokenId}.json for token IDs 1..24."
    }
  };
