const LOCATION_MAP = [
  ...
  { needle: "mallorca", tz: "Europe/Madrid", lat: 39.57, lon: 2.65 },
  { needle: "majorca", tz: "Europe/Madrid", lat: 39.57, lon: 2.65 },
  { needle: "palma", tz: "Europe/Madrid", lat: 39.57, lon: 2.65 },
  ...
  { needle: "boston", tz: "America/New_York", lat: 42.36, lon: -71.058 },
  { needle: "miami", tz: "America/New_York", lat: 25.761, lon: -80.192 },
  { needle: "chicago", tz: "America/Chicago", lat: 41.878, lon: -87.63 },
  { needle: "austin", tz: "America/Chicago", lat: 30.267, lon: -97.743 },
  { needle: "denver", tz: "America/Denver", lat: 39.739, lon: -104.99 },
  ...
  { needle: "portland", tz: "America/Los_Angeles", lat: 45.515, lon: -122.68 },
  ...
  { needle: "dublin", tz: "Europe/Dublin", lat: 53.35, lon: -6.26 },
  { needle: "berlin", tz: "Europe/Berlin", lat: 52.52, lon: 13.405 },
  ...
  { needle: "mallorca", tz: "Europe/Madrid", lat: 39.57, lon: 2.65 },
  { needle: "palma", tz: "Europe/Madrid", lat: 39.57, lon: 2.65 },
  { needle: "barcelona", tz: "Europe/Madrid", lat: 41.39, lon: 2.17 },
  ...
  { needle: "shanghai", tz: "Asia/Shanghai", lat: 31.23, lon: 121.474 },
  { needle: "hong kong", tz: "Asia/Hong_Kong", lat: 22.32, lon: 114.17 },
  { needle: "singapore", tz: "Asia/Singapore", lat: 1.352, lon: 103.82 },
  ...
  { needle: "são paulo", tz: "America/Sao_Paulo", lat: -23.55, lon: -46.633 },
  { needle: "sao paulo", tz: "America/Sao_Paulo", lat: -23.55, lon: -46.633 },
...
function labelFromCollaborator(c) {
  if (!c.location || c.location.toLowerCase() === "cloud") {
    return c.vendor ? `Cloud · ${c.vendor}` : "Cloud";
  }
  return c.location.split(",")[0].trim();
}
function resolveZones(clock, collaborators) {
  const out = /* @__PURE__ */ new Map();
  const push = (tz, label, sublabel, lat, lon, origin, name, extra) => {
    const key = `${origin}:${tz}:${label}`;
    const existing = out.get(key) ?? (origin === "collab" ? out.get(`collab:${tz}`) : void 0);
    if (existing) {
  ...
  if (clock?.includeCollaborators !== false) {
    for (const c of collaborators) {
      ...
      push(entry.tz, label, void 0, entry.lat, entry.lon, "collab", c.name);
    }
  }
  for (const z of clock?.zones ?? []) {
    ...
    push(z.tz, z.label, z.sublabel, lat, lon, "manual", void 0, {
      region: z.region,
      tags: z.tags,
      facts: z.facts,
      rituals: z.rituals,
