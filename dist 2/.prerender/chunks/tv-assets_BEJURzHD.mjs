...
const TV_STATION_NAV = STATIONS.map((station, index) => ({
  label: station.name,
...
function getTvAssetInventory() {
  const files = walk(PUBLIC_ROOT).map((filePath) => normalizePath(relative(PUBLIC_ROOT, filePath))).filter((publicPath) => {
    const extension = extname(publicPath).toLowerCase();
    return PUBLIC_EXTENSIONS.has(extension);
  }).map(toAsset).sort((a, b) => {
...
function countFiles(pathFromRoot, extensions) {
...
function groupForPath(publicPath, kind) {
  ...
  if (publicPath.startsWith("images/listening-room/") || publicPath.startsWith("images/houseplants/") || publicPath.startsWith("sitting-with-gandalf/") || publicPath === "images/relay-super-map.png") {
    return "rooms";
  }
  if (publicPath.startsWith("images/nouns-cola/") || publicPath.startsWith("images/nouns-cola-crush/") || publicPath.startsWith("images/paddle")) {
    return "product-art";
  }
...
function titleFromPath(publicPath) {
  ...
  return name.replace(/^\d+-/, "").replace(/[_-]+/g, " ").replace(/\bog\b/i, "OG").replace(/\btv\b/i, "TV").replace(/\bjson\b/i, "JSON").replace(/\bcss\b/i, "CSS").replace(/\bjs\b/i, "JS");
}
function normalizePath(path) {
  return path.split("\\").join("/");
}

export { TV_ASSET_GROUPS as T, TV_PRIMARY_NAV as a, TV_STATION_NAV as b, TV_SURFACE_NAV as c, TV_SOURCE_RUNS as d, getTvAssetInventory as g };
