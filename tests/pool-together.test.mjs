...
test('Pool Together wires the free memo desk, the paid memo action, and the pledge desk', async () => {
  ...
  assert.match(store, /INSERT INTO pool_together_memos/);
  const migration = await read('migrations/auth/0019_pool_together.sql');
  for (const table of ['pool_together_pledges', 'pool_together_nonces', 'pool_together_memos']) assert.match(migration, new RegExp(`CREATE TABLE ${table}`));
  assert.match(free, /verifyAgentRequest\(db, request/);
