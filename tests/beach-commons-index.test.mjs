...
test('Beach Commons is a twelve-edition series with four balanced paths', async () => {
  ...
  assert.equal(assignedPaths.length, 12);
  for (const path of ['build', 'make', 'kit', 'coast']) {
    assert.equal(assignedPaths.filter((match) => match[1] === path).length, 3);
  }

  assert.match(data, /currentEdition: 12/);
...
test('Beach Commons front door publishes a bounded machine index and discovery trail', async () => {
  ...
  assert.match(sitemap, /pointcast\.xyz\/beach-commons\/v12\.json/);
  ...
  assert.match(home, /All twelve Beach Commons editions/);
