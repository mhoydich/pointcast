...
test('V17 has JSON, Block, series, homepage, and discovery twins', async () => {
  ...
  assert.match(series, /currentEdition: 17/);
  ...
  assert.match(homepage, /href="\/beach-commons\/v17"/);
  assert.match(homepage, /Block 0546/);
  assert.match(homeEdition, /href="\/beach-commons\/v17"/);
  assert.match(homeEdition, /New<br \/>0546/);
  assert.match(indexPage, /Foundation → room → cart → proof\./);
});
