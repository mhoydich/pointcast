...
test('campaign desk has human, JSON, book, share-kit, and discovery surfaces', async () => {
  ...
  assert.match(promoData, /pointcast\.digital-pets-promo\/v2/);
  assert.match(promoData, /status: 'launched'/);
  assert.match(promoData, /https:\/\/x\.com\/mhoydich\/status\/2081936870641205589/);
  assert.match(book, /href="\/digital-pets\/share"/);
  assert.match(book, /href="\/digital-pets\/office"/);
  assert.match(shareKit, /key: 'digital-pets'/);
  assert.match(shareKit, /id: 'digital-pets-office'/);
  ...
  assert.match(forAgents, /\/digital-pets\/office\.json/);
  assert.match(llms, /book’s campaign desk/);
  assert.match(llms, /public six-week/);
  assert.match(llmsFull, /owned promotion desk/);
  assert.match(llmsFull, /public operating office/);
});
