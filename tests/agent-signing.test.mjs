...
const {
  ...
  SPEC_VERSION,
  MANIFEST_FIELDS,
} = await import(path.join(REPO_ROOT, 'src/lib/agent-signing.mjs'));
...
test('every resident in agent-identities.json has a working keypair', () => {
  const idJson = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'src/data/agent-identities.json'), 'utf8'));
