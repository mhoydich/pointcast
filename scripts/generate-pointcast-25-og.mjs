...
const boardFile = path.join(outDir, 'board-000.png');
...
await Promise.all([
  sharp(Buffer.from(svg)).png().toFile(boardFile),
  sharp(Buffer.from(disagreementSvg)).png().toFile(disagreementFile),
]);
process.stdout.write(`${boardFile}\n${disagreementFile}\n`);
