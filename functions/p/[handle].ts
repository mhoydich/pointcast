...
function profileHtml(profile: any, dogs: any[], seals: any[]): string {
  ...
  const dogRows = dogs.map((dog) => `<li><a href="${escapeHtml(dog.objktUrl)}" rel="noopener"><img src="${escapeHtml(ipfs(dog.image))}" alt="${escapeHtml(dog.name)}"><span>${escapeHtml(dog.name)}</span></a></li>`).join('') || '<li>None held by this owner.</li>';
