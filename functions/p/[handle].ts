...
import { listOwnerKennelDogs, readProfileHandle } from '../../src/lib/profile-object.mjs';
import { readSealShelf, type SealShelfResult } from '../../src/lib/seal-shelf';
...
function profileHtml(profile: any, dogs: any[], sealShelf: SealShelfResult): string {
  ...
  const sealRows = sealShelf.state === 'unavailable'
    ? '<li class="unavailable">Seal index unavailable. No absence is inferred.</li>'
    : sealShelf.groups.map((group) => `<li class="${group.active ? '' : 'revoked'}"><a href="${escapeHtml(group.representative.tzktUrl)}" rel="noopener"><strong>${escapeHtml(group.kind)}</strong><span>${group.total} on-chain ${group.total === 1 ? 'receipt' : 'receipts'} · ${group.active} active</span><small>${group.active ? `${group.active} active` : 'all revoked'}</small></a></li>`).join('') || '<li>None held by this owner.</li>';
...
export const onRequestGet: PagesFunction<Cloudflare.Env> = async ({ params }) => {
  ...
  try {
    ...
    const [dogs, sealShelf] = await Promise.all([
      listOwnerKennelDogs(profile.owner, contracts.kennel_club.mainnet).catch(() => []),
      readSealShelf(profile.owner),
    ]);
    if (json) return Response.json({ ...profile, dogs, seals: sealShelf.seals, sealShelf }, { headers: { 'Cache-Control': 'public, max-age=30, s-maxage=60', 'Access-Control-Allow-Origin': '*' } });
    return new Response(profileHtml(profile, dogs, sealShelf), { headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=30, s-maxage=60' } });
  } catch (error) {
