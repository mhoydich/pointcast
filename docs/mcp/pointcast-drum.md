...
| `nouns_battler_claim_board` | `{ taskId?, claimantName?, gang?, status?, proofUrl?, note?, participantKind? }` | Public claim card, proof checklist, production handoff, and participant-credit routing |
...
| `nouns-battler://claim-board` | `application/json` | Claim Board stance, public task cards, proof checklists, production handoff routes, and participant-credit routing |
