...
1. **One poster plate for HELLO.** Replace the placeholder at `public/images/faucet/hello-poster.png` (+ `.webp`, + the 1200×630 `hello-og.png`) with an original plate from the poster-image-engine. Direction from Mike: positive, Hockney-and-Warhol HELLO, a little Degas, finished monochrome.
...
2. `spigotSecretKey` shape check (`0x` + 64 hex). Should we also refuse a key whose address equals the HELLO deployer, as a tripwire against Mike pasting the 2019 key?
