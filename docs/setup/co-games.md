...
`/co-games/flow` opens with **Noun Drip: Heartfire Parade**. Collect 12 sparks in 35 seconds: every Noun pop earns one spark, with phase changes at 4, 8 and 12. The twelfth pop earns level completion and starts a bonus parade for the remaining music; an idle or incomplete run offers an encouraging retry rather than a win. Each different Noun joins a decorative parade, bounded to the eight authentic roster friends. Worlds remain freely selectable.

Nouns float down and wait on the canvas: tap any visible Noun at any height, or press D/F/J (1/2/3) to pop a Noun in that column. Native buttons also support Tab and Enter/Space. Distinct quick taps count independently; each Noun can be collected once. There are no missed-note penalties or health loss. Each shower holds at most six Nouns. Pause, menus, hidden pages, and switching modes stop the active shower and audio. Reduced motion settles the Nouns without decorative travel. The generated Heartfire courtyard appears only in the first Drip world; the other three story backgrounds and Rhythm mode retain their original scenes.
...
The Sound and feel dialog includes **Test sound**, an original three-second welcome tune with native audio controls and a direct media link. It does not start or advance a battle. STARJAM plays twelve original, finite 35-second AAC tracks (four worlds × three paces) through HTMLMediaElement, without routing them through Web Audio. Each arrangement develops through a spacious opening (0–8s), a fuller groove (8–17s), a rising section (17–25s), and a final lift (25–35s), preserving the original beat grid and finite media clock. Bright plucked melodies, harmonic bass and dry percussion are designed for phone speakers. Source and decoded-signal measurements are in `scripts/generate-starjam-soundtracks.py` and `/audio/starjam/manifest.json`; delivered audio totals about 3.53 MB across 17 assets, with only the selected backing and small accents loaded per visit. No external samples or AI/music-service requests are used.
...
Noun Drip accents use the same native AAC elements, authorized by a matching trusted Noun tap or lane key. The twelfth accepted pop adds one 1.5-second `level-clear.m4a` fanfare over the backing without seeking or changing its clock. Pause, mute, zero volume, menus, hidden pages, reset, mode changes and cleanup cancel the cue; Resume never replays it. A failed optional cue does not pause gameplay. Duplicate pop events and stale input cannot replay an accent. Mode changes stop the previous controller before mounting the next.

Cloudflare Pages currently [returns full 200 responses for byte-range requests](https://developers.cloudflare.com/pages/configuration/serving-pages/#behavior). A scoped middleware fallback serves correct 206 slices for the seventeen known STARJAM AAC files, preserving audio MIME, exact Content-Range and Content-Length. The fallback bounds actual reads to the declared asset length (at most 512 KiB), using verified manifest sizes if the Function's upstream response omits Content-Length. It supports prefix, open-ended and suffix ranges, and preserves existing partial/error responses. Strong If-Range ETags are checked; unvalidated conditions send the full file. Other PointCast resources retain their existing delivery behavior.
