...
Restore route pageview measurement as an anonymous 1-in-10 sample after a 20-view per-route, per-isolate daily warmup. Warmup views carry `sampled: 1`; later retained pageviews carry `sampled: 10`. This prevents low-traffic routes from becoming invisible without introducing a per-request KV counter. An isolate batches retained analytics records for up to 10 seconds into one KV value. `scripts/score-live.mjs` expands the weighted records when building the project score inputs.
