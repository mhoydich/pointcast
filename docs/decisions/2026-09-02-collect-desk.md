...
4. The button URL is `/k/today?claim=1&t=<per-subscriber-token>`.
5. `/k/today` resolves only a confirmed subscriber, issues the existing email identity’s session, and redirects to `/collect?claim=1`.
...
The subscriber token is a high-entropy bearer credential stored only in D1 and the person’s email. It is never included in public JSON or logs. Unsubscribing immediately makes it unusable for daily entry.
...
- `token` — unique high-entropy daily-entry and unsubscribe bearer token
