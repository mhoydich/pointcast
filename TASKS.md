...
- [x] (X) Commerce: ensure outbound checkout links strip referrers (`rel="noopener noreferrer"` + `referrerpolicy="no-referrer"`) on `/shop`, `/products/{slug}`, and `/pairings/{mood}` — `done` — completed by hourly commerce sprint 2026-07-11
